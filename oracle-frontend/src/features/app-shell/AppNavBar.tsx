...
export function AppNavBar() {
  ...
  return (
    <div className="sticky top-0 z-30 flex shrink-0 flex-col">
      ...
      <header className="flex h-14 items-center justify-between border-b border-line-2 bg-app-canvas px-6">
        <Link to="/app" className="flex shrink-0 items-center gap-2.5">
          <img src="/oracle-logo.png" alt="" className="size-6 object-contain" />
          <span className="text-[15px] font-medium tracking-tight text-ink-1">The Oracle</span>
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2.5">
          ...
          <WalletStatusButton className="flex cursor-pointer items-center gap-1.5 rounded-md border border-line-4 px-3 py-1.5 font-mono text-xs text-ink-5 hover:text-ink-1" />
        </div>
      </header>

      {/* Frame 2: primary nav — squared, bordered cards, same treatment as the wallet picker */}
      <nav className="flex h-[60px] flex-wrap items-center justify-center gap-2.5 border-b border-line-2 bg-panel-2 px-6">
        <SparNavItem />
